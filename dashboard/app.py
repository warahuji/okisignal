...
### 損切り（SL）・利確（TP）・ロットの決め方

#### 損切り（SL）
ATR（直近14本の平均的な値動き幅）× SLMult倍で自動計算。
例: ATR=0.30円、SLMult=0.75 → SL距離 = 0.225円（22.5pips）

#### 利確目標（TP1・TP2）
同じくATRベース。TP1 = ATR × TP1Mult、TP2 = ATR × TP2Mult。
例: TP1Mult=2.50 → +75pips、TP2Mult=3.50 → +105pips

#### ロットサイズ
「このSL距離で負けた場合、残高の1%だけ失う」ロットを自動計算。
例: 残高10万円 → 許容損失1,000円 → 1ロットで22,500円損なら → 0.04ロット

#### 具体例（USDJPY#）
```
残高10万円 / ATR=0.30円の場合:
  SL: -22.5pips（損切り）
  TP1: +75pips（半分利確）→ SLをBEに移動
  TP2: +105pips（残り利確）
  ロット: 0.04
  リスクリワード比: 1:3.3〜1:4.7
```
""")
